        ...
        while (true) {
            try {
                ...
                if (message.startsWith("-auth")) {
                    ...
                    Optional<Users> mayBeCredentials = chatServer.getAuthenticationService()
                    ...
                    if (mayBeCredentials.isPresent()) {
                        ...
                        if (!chatServer.isLoggedIn(credentials.getLogin())) {
                            name = credentials.getLogin();
                            chatServer.broadcast(String.format("User[%s] entered the chat", name));
                            chatServer.subscribe(this);
                            return;
                        } else {
                            sendMessage(String.format("User with name %s is already logged in", credentials.getLogin()));
                        }
                    } else {
